...
public class AndroidServiceCallerProxy extends ServiceCaller {
    ...
    private final static String TAG = AndroidServiceCallerProxy.class.getCanonicalName();
    ...
    public AndroidServiceCallerProxy(AndroidServiceBus androidServiceBus, boolean registerService,
	    String packageName, String serviceRequestGroundingID, String androidUniqueName,
	    ServiceRequestGroundingXmlObj serviceRequestGrounding, Context context) {
	super(androidServiceBus, false);
    ...
    public AndroidServiceCallerProxy(AndroidServiceBus androidServiceBus, String packageName,
	    String serviceRequestGroundingID, String androidUniqueName,
	    ServiceRequestGroundingXmlObj serviceRequestGrounding, Context context, String memberID) {
	this(androidServiceBus, false, packageName, serviceRequestGroundingID, androidUniqueName,
		serviceRequestGrounding, context);
	...
	if (null == waitingCall) {
	    Log.d(TAG, "No waiting call [" + reqID
		    + "] was found! Since the requestor doesn't wait for response!");
	    return;
	}

	// Get the action itself
	ActionXmlObj action = serviceRequestGrounding.getActionByName(waitingCall.getActionName());
    ...
    private void populateIntentWithOutputs(ServiceResponse response, ActionXmlObj action,
	...
	for (RequiredOutputXmlObj requiredOutput : action.getRequiredOutputs()) {
	    ...
	    String androidParamJavaClass = requiredOutput.getAndroidExtraParameterJavaClass();
	    String androidParameterName = requiredOutput.getAndroidExtraParameter();
	    if (List.class.getCanonicalName().equals(androidParamJavaClass)) {
		// This is a list
		populateIntentWithAListOutput(resultList, responseIntent, androidParameterName);
	    } else if (Bundle.class.getCanonicalName().equals(androidParamJavaClass)) {
		// This suppose to be mapped into android bundle
		// TODO: validate that the resultList has object, if not throw
		// an exception
		populateIntentWithABundleOutput((Resource) resultList.get(0), responseIntent,
			androidParameterName, requiredOutput.getBundleProperties());
	    } else {
		// Treat this as a string
		populateIntentWithAStringOutput(resultList.get(0), responseIntent,
			androidParameterName);
	    }
	}
    }
    
    private void populateIntentWithAListOutput(List resultList, Intent responseIntent,
	    String androidExtraParameterName) {
	String[] resultAsStringArr = new String[resultList.size()];
	for (int i = 0; i < resultList.size(); i++) {
	    resultAsStringArr[i] = extractStringFromOutputObject(resultList.get(i));
	}
    ...
    private void populateIntentWithABundleOutput(Resource resource, Intent responseIntent,
	    String androidParameterName, List<BundlePropertyXmlObj> bundleProperties) {
	// Build the bundle
	...
	for (BundlePropertyXmlObj bundleProperty : bundleProperties) {
	    ...
	    String propertyValue = String.valueOf(resource.getProperty(propertyUri));
	    ...
	    if (null != parser) {
		if (propertyValue.contains(parser.getAndroidNameWithoutParameter())) {
		    String androidValue = propertyValue.substring(parser
			    .getAndroidNameWithoutParameter().length(), propertyValue.length());
    ...
    private void populateIntentWithAStringOutput(Object object, Intent responseIntent,
	    String androidParameterName) {
	...
	while (i++ < NUM_OF_AMOUNTS_TO_GET_CALL) {
	    waitingCall = ((AndroidServiceBus) bus).getWaitingCall(reqID);
	    if (null != waitingCall) {
		break;
	    }
	    try {
	...
	ActionXmlObj actionXml = serviceRequestGrounding.getActionByName(actionName);
	...
	try {
	    ...
	    Log.e(TAG,
		    "Error when creating service request for action ["
	...
	if (null != callID) {
	    ...
	    String replyToActionParameterName = actionXml.getAndroidReplyToAction();
	    ...
	    String replyToCategoryParameterName = actionXml.getAndroidReplyToCategory();
	...
	throw new UnsupportedOperationException("This method should not be called for ["
		+ AndroidServiceCallerProxy.class.getCanonicalName());
    }

    private ServiceRequest createServiceRequest(ActionXmlObj actionXml,
	    Bundle extrasSectionOfOriginalServiceCallerRequest) throws Exception {
	Service service = null;
	try {
	    // Create the service class
	    Class serviceJavaClass = Class.forName(actionXml.getServiceJavaClass());
	    service = (Service) serviceJavaClass.newInstance();
	} catch (Throwable th) {
	    String errMsg = "Unable to create java class [" + actionXml.getServiceJavaClass()
    ...
    private void populateValueFilters(ServiceRequest serviceRequest,
	    List<ValueFilterXmlObj> valueFilters, Bundle extrasSectionOfOriginalServiceCallerRequest)
	    throws Exception {
	for (ValueFilterXmlObj valueFilter : valueFilters) {
	    ...
	    Constructor ctor = ReflectionsUtils.createCtorThatReceiveStringParam(valueFilter
		    .getJavaClass());
	    Object object = ReflectionsUtils.invokeCtorWithStringParam(ctor, parameterValue);
	...
	for (ChangeEffectXmlObj changeFilter : changeEffects) {
	    ...
	    Object value;
	    try {
		value = TypeMapper.getJavaInstance(changeFilter.getValue().getValue(), TypeMapper
			.getDatatypeURI(Class.forName(changeFilter.getValue().getJavaClass())));
	    } catch (ClassNotFoundException e) {
		String errMsg = "Unable to create java class ["
			+ changeFilter.getValue().getJavaClass() + "] due to [" + e.getMessage()
			+ "]";
		throw new Exception(errMsg);
	    }
