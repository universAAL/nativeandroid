...
public class RequiredOutputXmlObj extends AbstractParameterXmlObj {
	...
	uri = CommonXmlParserUtils.getAttributeValueAsString(requiredOutputNode,
		IServiceRequestGroundingXmlConstants.REQUIRED_OUTPUT_ATTRIBUTE_URI);

	if (Bundle.class.getCanonicalName().equals(androidExtraParameterJavaClass)) {
	...
	Node bundlePropertiesNode = CommonXmlParserUtils.getSingleChildNode(requiredOutputNode,
		ICommonServiceBusXmlConstants.BUNDLE_PROPERTIES_ELEMENT);

	// Get the BundlePeroperty children
	List<Node> bundlePropertyChildrenList = CommonXmlParserUtils.getChildNodes(
		bundlePropertiesNode, ICommonServiceBusXmlConstants.BUNDLE_PROPERTY_ELEMENT);
	for (Node curBundlePropertyChild : bundlePropertyChildrenList) {
	    bundleProperties.add(new BundlePropertyXmlObj(curBundlePropertyChild));
	}
    }
}
