 ...
 * 
 */
public class NoticeJoiningPeerHandler extends AbstractMessagePersistableHandler {

    @Override
    protected void privateHandleMessage(IMessage message, AbstractSodaPopAndroidImpl sodaPop) {
	// Cast it to notice new peer message
	NoticeNewPeerMessage noticeNewPeerMessage = (NoticeNewPeerMessage) message;

	// Persist the new peer
	sodaPop.noticeNewPeer(noticeNewPeerMessage.getPeerID(), noticeNewPeerMessage.getProtocol());
    }
}
