...
public abstract class AbstractMessage implements IMessage {
    ...
    private final static String TAG = AbstractMessage.class.getCanonicalName();
	...
	try {
	    ...
	    Log.e(TAG, "Error when handling the message due to [" + th.getMessage() + "]");
	}
    }
	...
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_PROTOCOL);
    }

    protected String extractPeerIDFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_PEER_ID);
    }

    protected String extractPeerBussesFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_PEER_BUSSES);
    }

    protected String extractPeerBusNameFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_PEER_BUS_NAME);
    }

    protected String extractBusPackageNameFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_BUS_PACKAGE_NAME);
    }

    protected String extractBusClassNameFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_BUS_CLASS_NAME);
    }

    protected String extractRemotePeerIDsForResponseFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_REMOTE_PEER_IDS_RSP);
    }

    protected String extractPackageNameFromExtras() {
	return extractStringFromExtras(IAndroidSodaPop.EXTRAS_KEY_PACKAGE_NAME);
    }
