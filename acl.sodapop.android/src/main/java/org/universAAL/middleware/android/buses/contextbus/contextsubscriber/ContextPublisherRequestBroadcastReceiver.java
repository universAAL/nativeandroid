...
public class ContextPublisherRequestBroadcastReceiver extends BroadcastReceiver {
    ...
    public ContextPublisherRequestBroadcastReceiver(String contextPublisherID, String androidAction) {
	...
	Log.d(TAG,
		"ContextSubscriberRequestBroadcastReceiver received Intent [" + intent.toString()
			+ "]; ContextPublisherID [" + contextPublisherID + "]");
	...
	androidContextBusIntent.setClass(context, AndroidContextBusService.class);

	// Change the action
	androidContextBusIntent.setAction(IAndroidSodaPop.ACTION_CONTEXT_PUBLISHER_REQUEST);

	// Store the context publisher ID
	androidContextBusIntent.putExtra(IAndroidSodaPop.EXTRAS_KEY_CONTEXT_PUBLISHER_ID,
		contextPublisherID);

	// Store the android action (the action that is related to the caller)
	androidContextBusIntent.putExtra(IAndroidSodaPop.EXTRAS_KEY_ACTION_NAME, androidAction);

	// Add protocol // TODO: this should be handled in more generic way
	androidContextBusIntent.putExtra(IAndroidSodaPop.EXTRAS_KEY_PROTOCOL,
	...
	boolean unreigster = intent.getBooleanExtra(
		IAndroidSodaPop.EXTRAS_KEY_UNREGISTER_BROADCAST_RECEIVER, false);
