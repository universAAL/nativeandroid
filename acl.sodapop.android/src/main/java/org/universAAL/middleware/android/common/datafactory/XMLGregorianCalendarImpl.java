 ...
 * The changes are:
 * 	1. The package name changed to org.universAAL.middleware.android.common.datafactory
 * 	2. Code that uses 'DatatypeMessageFormatter' was commented
 * It is copied since this type is missing on Android.
 * Retrieved from 
 * http://svn.apache.org/repos/asf/xerces/java/tags/Xerces-J_2_9_1/src/org/apache/xerces/jaxp/datatype/XMLGregorianCalendarImpl.java
 * on 05.05.2012
 */
/*
...
class XMLGregorianCalendarImpl extends XMLGregorianCalendar implements Serializable, Cloneable {
     ...
     * 
     * <p>
     * TODO: Serialization should use the XML string representation as the
     * serialization format to ensure future compatibility.
     * </p>
     */
     ...
     */
    public static final XMLGregorianCalendar LEAP_YEAR_DEFAULT = createDateTime(400, // year
	...
	if (lexRep.indexOf('T') != NOT_FOUND) {
		...
		try {
			...
			// DatatypeMessageFormatter.formatMessage(null,"InvalidXGCRepresentation",
			// new Object[]{lexicalRepresentation})
     ...
     * 
     */
    protected XMLGregorianCalendarImpl(BigInteger year, int month, int day, int hour, int minute,
     ...
     *            of <code>XMLGregorianCalendar</code> to be created.
     */
    private XMLGregorianCalendarImpl(int year, int month, int day, int hour, int minute,
	...
	if (!isValid()) {
	    ...
	    // new Object[] { new Integer(year), new Integer(month), new
	    // Integer(day),
	    // new Integer(hour), new Integer(minute), new Integer(second),
	...
	int offsetInMinutes = (cal.get(Calendar.ZONE_OFFSET) + cal.get(Calendar.DST_OFFSET))
		/ (60 * 1000);
	...
	return new XMLGregorianCalendarImpl(year, month, day, hours, minutes, seconds,
     ...
     * @see DatatypeConstants#FIELD_UNDEFINED
     */
    public static XMLGregorianCalendar createDateTime(int year, int month, int day, int hour,
     ...
     * @see DatatypeConstants#FIELD_UNDEFINED
     */
    public static XMLGregorianCalendar createDateTime(int year, int month, int day, int hours,
	    int minutes, int seconds, int milliseconds, int timezone) {
	...
	return new XMLGregorianCalendarImpl(year, month, day, DatatypeConstants.FIELD_UNDEFINED, // hour
	...
	// unknown
	throw new IllegalStateException(this.getClass().getName() + "#getXMLSchemaType() :"
	// + DatatypeMessageFormatter.formatMessage(null, "InvalidXGCFields",
	// null)
	);
    }
	...
	BigInteger carry = new BigDecimal(temp.subtract(BigInteger.ONE)).divide(
		new BigDecimal(TWELVE), BigDecimal.ROUND_FLOOR).toBigInteger();
	...
	BigDecimal fQuotient = new BigDecimal(new BigDecimal(tempBD.toBigInteger()).divide(
		DECIMAL_SIXTY, BigDecimal.ROUND_FLOOR).toBigInteger());
	...
	BigDecimal tempFracSeconds = endSeconds.subtract(new BigDecimal(BigInteger
		.valueOf((long) getSecond())));
	...
	while (true) {
		...
		quotient = BigDecimal.valueOf(intTemp - 1)
			.divide(new BigDecimal(TWELVE), BigDecimal.ROUND_UP).intValue();
	    } else {
	...
	if (month != DatatypeConstants.FEBRUARY) {
	    ...
	    if (year.mod(FOUR_HUNDRED).equals(BigInteger.ZERO)
		    || (!year.mod(HUNDRED).equals(BigInteger.ZERO) && year.mod(FOUR).equals(
			    BigInteger.ZERO))) {
		// is a leap year.
		return 29;
	    } else {
	...
	if (year != DatatypeConstants.FIELD_UNDEFINED) {
		...
		result.set(Calendar.ERA, eonAndYear.signum() == -1 ? GregorianCalendar.BC
	...
	if (year != DatatypeConstants.FIELD_UNDEFINED) {
		...
		if (defaultYear != DatatypeConstants.FIELD_UNDEFINED) {
			...
			result.set(Calendar.ERA, defaultYear < 0 ? GregorianCalendar.BC
				: GregorianCalendar.AD);
	...
	return new XMLGregorianCalendarImpl(getEonAndYear(), this.month, this.day, this.hour,
		this.minute, this.second, this.fractionalSecond, this.timezone);
    }
