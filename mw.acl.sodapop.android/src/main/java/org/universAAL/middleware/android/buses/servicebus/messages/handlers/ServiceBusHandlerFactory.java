...
public class ServiceBusHandlerFactory {
    ...
    private static final String TAG = ServiceBusHandlerFactory.class.getCanonicalName();
	...
	case INITIALIZE:
	    ...
	    break;
	case REGISTER:
	    handler = new RegisterServicesHandler();
	    break;
	case UNREGISTER:
	    handler = new UnregisterServicesHandler();
	    break;
	case PROCESS_BUS_MESSAGE:
	    handler = new ProcessServiceBusMessageRequestHandler();
	    break;
	case PROCESS_BUS_MESSAGE_RSP:
	    handler = new ProcessBusMessageResponseHandler();
	    break;
