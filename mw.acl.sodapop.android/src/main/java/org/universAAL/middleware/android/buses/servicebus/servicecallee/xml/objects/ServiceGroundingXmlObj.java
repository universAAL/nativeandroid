...
public class ServiceGroundingXmlObj extends OntologiesContainedXml {
    ...
    public ServiceGroundingXmlObj(Node serviceNode, AndroidServiceType androidServiceType) {
	...
	superJavaClass = CommonXmlParserUtils.getAttributeValueAsString(serviceNode,
		IServiceGroundingXmlConstants.SERVICE_GROUNDING_ATTRIBUTE_SUPER_JAVA_CLASS);

	// Get the version
	version = CommonXmlParserUtils.getAttributeValueAsString(serviceNode,
	...
	return (ServiceGroundingXmlObj) SerializeUtils.deserializeObject(objAsBytesArr);
    }
	...
	Node operationsNode = CommonXmlParserUtils.getSingleChildNode(serviceNode,
		IServiceGroundingXmlConstants.OPERATIONS_ELEMENT);

	// Get the operation list
	List<Node> operationsChileNodes = CommonXmlParserUtils.getChildNodes(operationsNode,
