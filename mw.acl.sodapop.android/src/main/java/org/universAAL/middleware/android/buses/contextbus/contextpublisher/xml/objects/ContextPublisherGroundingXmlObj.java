...
public class ContextPublisherGroundingXmlObj implements Serializable {
	...
	// Get the uri
	uri = CommonXmlParserUtils.getAttributeValueAsString(contextPublisherNode,
    ...
    public static ContextPublisherGroundingXmlObj deserialize(byte[] objAsBytesArr) {
	return (ContextPublisherGroundingXmlObj) SerializeUtils.deserializeObject(objAsBytesArr);
    }

    private void populateActions(Node contextPublisherNode) {
	// Get the actions element
	Node actionsNode = CommonXmlParserUtils.getSingleChildNode(contextPublisherNode,
		IContextPublisherGroundingXmlConstants.ACTIONS_ELEMENT);

	// Get the action list
	List<Node> actionsChileNodes = CommonXmlParserUtils.getChildNodes(actionsNode,
