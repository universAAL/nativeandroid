...
public class AndroidServiceCalleeProxy extends ServiceCallee {
    ...
    private final static String TAG = AndroidServiceCalleeProxy.class.getCanonicalName();
    ...
    public AndroidServiceCalleeProxy(AndroidServiceBus androidServiceBus,
	    ServiceProfile[] realizedServices, String packageName, String serviceGroundingID,
	    String androidServiceName, ServiceGroundingXmlObj serviceGrounding, Context context) {
	super(androidServiceBus);
    ...
    public AndroidServiceCalleeProxy(AndroidServiceBus androidServiceBus, String packageName,
	    String serviceGroundingID, String androidServiceName,
	    ServiceGroundingXmlObj serviceGrounding, Context context, String memberID) {
	this(androidServiceBus, null, packageName, serviceGroundingID, androidServiceName,
		serviceGrounding, context);
    ...
    public ServiceResponse handleCall(Message message) {
	...
	OperationXmlObj foundOperation = serviceGrounding.getOperationByUri(uAALOperation);
	...
	if (null == foundOperation) {
	    ...
	    // Handle inputs
	    boolean doesItFitToCallee = populateIntentWithInputs(serviceCall, foundOperation, intent);

	    // Check if the operation supports a reply back
	    if (StringUtils.isEmpty(foundOperation.getAndroidReplyToAction()) ||
		    StringUtils.isEmpty(foundOperation.getAndroidReplyToCategory())) {
		...
		populateIntentWithOutputsAndRegisterRecevier(message, foundOperation, intent);
	    }
		...
		Log.i(TAG, "The request [" + message.getID() + "] doesn't fit to the callee, therefore ignore it");
	    }
	}
    ...
    public void handleResponse(String messageIDInReplyTo, String operationNameToRespondTo,
	...
	try {
		...
		switch (cardinality) {
		case Single:
		    String value = extras.getString(parser.getAndroidParameter());
		    List<Object> objectsExpectedSingleOne = populateOutputObjects(output,
			    new String[] { value }, parser);
		    sr.addOutput(new ProcessOutput(outputURI, objectsExpectedSingleOne.get(0)));
		    break;
		case Multi: // TODO: here you should take the type from the xml
			    // and by reflection initate it
		    String[] values = extras.getStringArray(parser.getAndroidParameter());
		    List<Object> objectsExpecteMulti = populateOutputObjects(output, values, parser);
		    sr.addOutput(new ProcessOutput(outputURI, objectsExpecteMulti));
		    break;
		}
	    }
    ...
    private List<Object> populateOutputObjects(OutputXmlObj output, String[] values,
	    AndroidNameParameterParser parser) throws Exception {
	List<Object> objects = new ArrayList<Object>();

	Constructor constructor = ReflectionsUtils.createCtorThatReceiveStringParam(output
		.getJavaClass());

	for (String value : values) {
	    Object obj = ReflectionsUtils.invokeCtorWithStringParam(constructor,
		    parser.getAndroidNameWithoutParameter() + value);
	    objects.add(obj);
	}
	...
	String replyTo = message.getSource();

	// Category
	String category = Intent.CATEGORY_DEFAULT;

	// Prepare the broadcast receiver
	registerRequestToBroadcastReceiver(messageIDAsAction, operation.getUri(), category, replyTo);
    ...
    private void registerRequestToBroadcastReceiver(String messageIDAsAction, String operationName,
	    String category, String replyTo) {

	// Initiate the receiver
	BroadcastReceiver receiver = new ServiceCalleeResponsesBroadcastReceiver(myID,
	...
	Log.d(TAG, "A registration to receiver has been performed: Action [" + messageIDAsAction
		+ "]; Category [" + category + "]; ReplyTo [" + replyTo + "]");
    }

    private static boolean populateIntentWithInputs(ServiceCall call, OperationXmlObj operation,
	    Intent intent) {
	// If not inputs - then for sure it will be fit to the callee
	boolean fitToCallee = operation.getFilteringInputs().size() > 0 ? false : true;
	...
	for (FilteringInputXmlObj input : operation.getFilteringInputs()) {
	    ...
	    if (null != parser) {
		if (inputValueAsString.contains(parser.getAndroidNameWithoutParameter())) {
		    String inputAndroidValue = inputValueAsString
			    .substring(parser.getAndroidNameWithoutParameter().length(),
				    inputValueAsString.length());

		    // Add the input value to the intent
		    intent.putExtra(parser.getAndroidParameter(), inputAndroidValue);
	...
	public static CardinalityType getCardinality(int minCardinality, int maxCardinality) {
