...
import org.universAAL.middleware.android.upnp.actions.ReplyPeerBussesActionInvocation;
import org.universAAL.middleware.android.upnp.exceptions.UnknownRemoteDeviceExeption;
import org.universAAL.middleware.android.upnp.messages.ExportReplyPeerBussesMessage;
 ...
 * 
 */
public class ExportReplyPeerBussesHandler extends AbstractMessageHandler {

    public void handleMessage(IMessage message) throws UnknownRemoteDeviceExeption {
	// Cast it to ReplyPeerBusses message
	ExportReplyPeerBussesMessage replyPeerBusses = (ExportReplyPeerBussesMessage) message;

	// Get the remote proxy peer
	Service<RemoteDevice, RemoteService> remoteService = RemoteServicesContainer.getInstance()
		.getRemoteService(replyPeerBusses.getRemotePeerIDForResponse());

	// Initiated the action
	ReplyPeerBussesActionInvocation action = new ReplyPeerBussesActionInvocation(remoteService,
		replyPeerBusses.getPeerID(), replyPeerBusses.getBusNames());

	// Get the context - it's the service
	UPnPSodaPopPeersService upnpSodaPopService = (UPnPSodaPopPeersService) replyPeerBusses
		.getContext();
