...
public class RegisterServicesHandler extends AbstractMessagePersistableHandler {
    ...
    private static final String TAG = RegisterServicesHandler.class.getCanonicalName();
	...
	if (registerServicesMessage.getAndroidServiceCalleesDataWrapperList().isEmpty()
		&& registerServicesMessage.getAndroidServiceCallersDataWrapperList().isEmpty()) {
	    Log.d(TAG, "Nothing was found to handle, therefore do nothing!");
	    return;
	}

	// Share the ModuleContext
	...
	AndroidServiceBus androidServiceBus = new AndroidServiceBus(sodaPop, context);
	...
	for (AndroidServiceCalleeDataWrapper serviceCalleeRegistration : registerServicesMessage
	    ...
	    handleServiceCallee(serviceCalleeRegistration, androidServiceBus, context);
	}
	...
	for (AndroidServiceCallerDataWrapper serviceCallerRegistration : registerServicesMessage
	    ...
	    handleServiceCaller(serviceCallerRegistration, androidServiceBus, context);
	}
    }

    protected void handleServiceCallee(AndroidServiceCalleeDataWrapper serviceCalleeRegistration,
	    AndroidServiceBus androidServiceBus, Context context) {

	ServiceGroundingXmlObj serviceGroundingXml = serviceCalleeRegistration
		.getServiceGroundingXmlObj();

	// Register ontologies
	registerOntologies(serviceGroundingXml);

	// Populate the Service Profiles
	ServiceProfile[] serviceProfiles = null;
	try {
	    serviceProfiles = populateServiceProfiles(serviceGroundingXml);
	} catch (Exception e) {
	    String errMsg = "Unable to register the service grounding [" + e.getMessage() + "]";
	    Log.e(TAG, errMsg);
	    return;
	}
	...
	// stuff
	new AndroidServiceCalleeProxy(androidServiceBus, serviceProfiles,
		serviceCalleeRegistration.getPackageName(),
		serviceCalleeRegistration.getGroundingID(),
		serviceCalleeRegistration.getAndroidUniqueName(), serviceGroundingXml, context);
    }

    private static void registerOntologies(ServiceGroundingXmlObj serviceXml) {
	registerOntologies(serviceXml.getOntologies());
    }

    private static ServiceProfile[] populateServiceProfiles(ServiceGroundingXmlObj serviceXml)
	    throws Exception {
	...
	class ServiceWrapper extends Service {
	    ...
	    @Override
	    public void addFilteringInput(String inParamURI, String typeURI, int minCardinality,
		    int maxCardinality, String[] propPath) {
		super.addFilteringInput(inParamURI, typeURI, minCardinality, maxCardinality,
			propPath);
	    }
		...
		for (FilteringInputXmlObj inputXml : operationXml.getFilteringInputs()) {
		    addFilteringInput(inputXml.getName(), inputXml.getFiltering().getClassURI(),
			    inputXml.getFiltering().getMinCardinality(), inputXml.getFiltering()
				    .getMaxCardinality(), inputXml.getPropertiesAsStringArr());
		}
	    }

	    void addOutputsToService(OperationXmlObj operationXml) throws Exception {
		for (OutputXmlObj outputXml : operationXml.getOutputs()) {
		    ProcessOutput out = new ProcessOutput(outputXml.getName());
		    out.setParameterType(ReflectionsUtils.extractMYURIFieldFromClass(outputXml
			    .getJavaClass()));
		    out.setCardinality(outputXml.getMinCardinality(), outputXml.getMaxCardinality());
	    ...
	    void addChangeEffectsToService(OperationXmlObj operationXml) throws Exception {
		for (ChangeEffectXmlObj changeEffectXml : operationXml.getChangeEffects()) {
			...
			value = TypeMapper.getJavaInstance(valueAsStr,
				TypeMapper.getDatatypeURI(Class.forName(valueCls)));
		    ...
		    getServiceProfile().addChangeEffect(changeEffectXml.getPropertiesAsStringArr(),
			    value);
		}
	    }
	}
	...
	String superJavaClass = ReflectionsUtils.extractMYURIFieldFromClass(serviceXml
		.getSuperJavaClass());

	AndroidOntologyManagement.registerOntology(new SimpleOntology(serviceXml.getUri(),
		superJavaClass, new ResourceFactoryImpl() {
    ...
    protected void handleServiceCaller(AndroidServiceCallerDataWrapper serviceCallerRegistration,
	    AndroidServiceBus androidServiceBus, Context context) {
	...
	new AndroidServiceCallerProxy(androidServiceBus, registerService,
		serviceCallerRegistration.getPackageName(),
		serviceCallerRegistration.getGroundingID(),
		serviceCallerRegistration.getAndroidUniqueName(),
		serviceCallerRegistration.getServiceRequestGroundingXmlObj(), context);
    }

    private void registerOntologies(ServiceRequestGroundingXmlObj serviceRequestGroundingXml) {
	registerOntologies(serviceRequestGroundingXml.getOntologies());
    }

    private static void registerOntologies(String[] ontologies) {

	ServiceBusOntology serviceOntology = new ServiceBusOntology();
	AndroidOntologyManagement.registerOntology(serviceOntology);

	// Register the ontologies that the service grounding uses
	for (String ontologyClass : ontologies) {
	    try {
		AndroidOntologyManagement.registerOntology(ontologyClass);
	    } catch (Throwable th) {
		String errMsg = "Error when registering ontology [" + ontologyClass + "] due to ["
			+ th.getMessage() + "]";
