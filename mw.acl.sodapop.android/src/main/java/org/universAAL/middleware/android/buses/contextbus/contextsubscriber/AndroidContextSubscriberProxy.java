 ...
 * 
 */
public class AndroidContextSubscriberProxy extends ContextSubscriber implements IGroundingIDWrapper {

    private static final String TAG = AndroidContextSubscriberProxy.class.getCanonicalName();
    ...
    public AndroidContextSubscriberProxy(AndroidContextBus androidContextBus, boolean register,
    ...
    public AndroidContextSubscriberProxy(AndroidContextBus androidContextBus, String packageName,
	    String groundingID, String androidUniqueName,
	    ContextSubscriberGroundingXmlObj contextSubscriberGrounding, Context context,
	    String memberID) {
	this(androidContextBus, false, null, packageName, groundingID, androidUniqueName,
		contextSubscriberGrounding, context);
	...
	if (!contextEvent.getRDFPredicate().equals(action.getPredicatePropName())) {
	    Log.w(TAG,
		    "Got context event with predicate [" + contextEvent.getRDFPredicate()
	...
	for (SubjectFilterXmlObj subjectFilter : action.getSubjectFilters()) {
	    ...
	    if (null != parser) {
		if (contextEventSubjectURI.contains(parser.getAndroidNameWithoutParameter())) {
		    String inputAndroidValue = contextEventSubjectURI.substring(parser
			    .getAndroidNameWithoutParameter().length(), contextEventSubjectURI
			    .length());
	...
	IntentUtils.addToExtra(action.getObject().getAndroidExtraParameter(), action.getObject()
		.getAndroidExtraParameterJavaClass(), contextEvent.getRDFObject(), intent);
