 ...
 * 
 */
public class AndroidLocalServicesIndexData extends AbstractAndroidServiceBusPersistable implements
	ILocalServicesIndexData {
	...
	sqliteMngr.open();
	try {
	    localServiceIndexRowDB = sqliteMngr.removeLocalServiceIndexByProcessURI(id);
	} finally {
	    sqliteMngr.close();
	}
	...
	sqliteMngr.open();
	try {
	    localServiceIndexRowDB = sqliteMngr.queryLocalServiceIndexByProcessURI(id);
	} finally {
	    sqliteMngr.close();
	}
	...
	for (int i = 0; i < localServiceIndexRowsDB.length; i++) {
	    serviceRealizationIds[i] = localServiceIndexRowsDB[i].getProcessURI();
	}
