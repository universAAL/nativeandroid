...
public class AndroidNameParameterParser {
    ...
    private static final String TAG = AndroidNameParameterParser.class.getCanonicalName();

    private String androidNameWithoutParameter;
    private String androidParameter;

    protected AndroidNameParameterParser(String androidNameWithoutParameter, String androidParameter) {
	...
	Log.d(TAG, "Is about to parse name [" + androidName + "] delims [" + startDelim + endDelim
		+ "]");
	...
	if (startIndex > -1 && endIndex > -1 && endIndex > startIndex) {
	    String androidNameWithoutParameter = androidName.substring(0, startIndex);
	    String androidParameter = androidName.substring(startIndex + startDelim.length(),
		    endIndex);

	    parser = new AndroidNameParameterParser(androidNameWithoutParameter, androidParameter);
	}
