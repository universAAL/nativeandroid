...
import org.universAAL.middleware.android.upnp.actions.ProcessBusMessageAction;
 ...
 * 
 */
public class ExportProcessBusMessageHandler extends AbstractMessageHandler {

    public void handleMessage(IMessage message) throws UnknownRemoteDeviceExeption {
	// Cast it to ProcessBusMessage message
	ExportProcessBusMessage processBusMessage = (ExportProcessBusMessage) message;

	// Get the context - it's the service
	UPnPSodaPopPeersService upnpSodaPopPeersService = (UPnPSodaPopPeersService) processBusMessage
		.getContext();

	// For each remote peer send a message
	for (String remotePeerID : processBusMessage.getRemotePeerIDs()) {
	    sendProcessBusMessageToRemotePeer(remotePeerID, processBusMessage.getBusName(),
		    processBusMessage.getMessage(), upnpSodaPopPeersService);
	}
    }

    private void sendProcessBusMessageToRemotePeer(String remotePeerID, String busName,
	    String message, UPnPSodaPopPeersService upnpSodaPopPeersService)
	...
	// Initiate the action
	ProcessBusMessageAction action = new ProcessBusMessageAction(remoteService, busName,
		message);
