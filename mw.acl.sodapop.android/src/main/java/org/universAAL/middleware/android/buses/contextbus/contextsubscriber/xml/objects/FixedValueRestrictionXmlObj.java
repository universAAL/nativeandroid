...
public class FixedValueRestrictionXmlObj extends AbstractRestrictionXmlObj {
	...
	// Get the Value node
	Node valueNode = CommonXmlParserUtils.getSingleChildNode(restrictionNode,
	...
	try {
	    ctor = ReflectionsUtils.createCtorThatReceiveStringParam(value.getJavaClass());
	    obj = ReflectionsUtils.invokeCtorWithStringParam(ctor, value.getValue());

	    MergedRestriction localMergedRestriction = MergedRestriction.getFixedValueRestriction(
		    propertyUri, obj);

	    return (null == mergedRestriction) ? localMergedRestriction : mergedRestriction
		    .addRestriction(localMergedRestriction);
	} catch (Throwable th) {
	    throw new IllegalGroundingFormat("Unable to add restrictions due to ["
		    + th.getMessage() + "]");
	}
    }
}
