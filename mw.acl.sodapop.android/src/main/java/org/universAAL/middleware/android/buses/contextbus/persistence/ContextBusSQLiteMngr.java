...
public class ContextBusSQLiteMngr extends AbstractCommonBusSQLiteMngr {
	...
	valuesForProvisionTable.put(ProvisionsTable.COLUMN_PROVISION_ID, provisionID);

	// Insert the new row to the Provisions table
	long insertedID = database
		.insert(ProvisionsTable.TABLE_NAME, null, valuesForProvisionTable);
	Log.d(TAG, "Inserted status [" + insertedID + "] to [" + ProvisionsTable.TABLE_NAME
		+ "] table");
	...
	Log.d(TAG, "Is about to query for Provision provisionID [" + provisionID + "]");

	ProvisionRowDB provisionRowDB = null;

	Cursor cursor = database.query(ProvisionsTable.TABLE_NAME, ProvisionsTable.allColumns,
		ProvisionsTable.COLUMN_PROVISION_ID + " = ?", new String[] { provisionID }, null,
		null, null);
	...
	if (!cursor.isAfterLast()) {
	    ...
	    Log.d(TAG, "No provision was found provision with ID [" + provisionID + "]");
	}
	...
	Cursor cursor = database.query(CalledPeersTable.TABLE_NAME, CalledPeersTable.allColumns,
		CalledPeersTable.COLUMN_MESSAGE_ID + " = ?", new String[] { messageID }, null,
		null, null);
	...
	if (!cursor.isAfterLast()) {
	    ...
	    Log.d(TAG, "No CalledPeers was found for messageID [" + messageID + "]");
	}
    ...
    public void updateCalledPeersWithNumOfCalledPeers(String messageID, int numOfCalledPeers) {
	ContentValues valuesForCalledPeers = new ContentValues();
	valuesForCalledPeers.put(CalledPeersTable.COLUMN_NUM_OF_CALLED_PEERS, numOfCalledPeers);

	int updatedRowsCount = database.update(CalledPeersTable.TABLE_NAME, valuesForCalledPeers,
		CalledPeersTable.COLUMN_MESSAGE_ID + "= ?", new String[] { messageID });
	if (1 != updatedRowsCount) {
	    Log.e(TAG, "Only one row should be found for the messageID [" + messageID
		    + "]; were found [" + updatedRowsCount + "] rows");
	} else {
	    Log.d(TAG, "CalledPeers with MessageID [" + messageID
		    + "] has been updated with numOfCalledPeers [" + numOfCalledPeers + "]");
	}
    }

    public void removeCalledPeers(String messageID) {
	Log.d(TAG, "Is about to remove CalledPeers with messageID [" + messageID + "]");

	int numOfDeletedRows = database.delete(CalledPeersTable.TABLE_NAME,
		CalledPeersTable.COLUMN_MESSAGE_ID + "= ?", new String[] { messageID });
	...
	long insertedID = database.insert(CalledPeersTable.TABLE_NAME, null, valuesForCalledPeers);
	Log.d(TAG, "Inserted status [" + insertedID + "] to [" + CalledPeersTable.TABLE_NAME
		+ "] table");
     ...
     * 
     */
    public void addContextEventPatterns(String messageID, String[] serializedContextEventPatterns) {
	...
	for (String contextEventPattern : serializedContextEventPatterns) {
	    insertContextEventPattern(calledPeersRow.getCalledPeerID(), contextEventPattern);
	}
    }

    public ContextEventPatternRowDB[] queryForContextEventPatterns(String messageID) {
	Log.d(TAG, "Is about to query for ContextEventPatterns for MessageID [" + messageID + "]");

	// Query for the CalledPeers to get the ID_PK
	CalledPeersRowDB calledPeersRow = queryForCalledPeers(messageID);

	return queryForContextEventPatternsByCalledPeersID(calledPeersRow.getCalledPeerID());
    }

    private long insertContextEventPattern(long calledPeerID, String contextEventPattern) {
	Log.d(TAG, "Is about to insert ContextEvent for CalledPeersID [" + calledPeerID
		+ "]; ContextEvent [" + contextEventPattern + "]");

	// Populate the values to insert
	ContentValues valuesForContextEventPattern = new ContentValues();
	valuesForContextEventPattern.put(ContextEventPatternsTable.COLUMN_CONTEXT_EVENT_CONTENT,
		contextEventPattern);
	valuesForContextEventPattern.put(ContextEventPatternsTable.COLUMN_CALLED_PEERS_FK,
		calledPeerID);

	// Insert the new row to the ContextEventPatterns table
	long insertedID = database.insert(ContextEventPatternsTable.TABLE_NAME, null,
    ...
    private ContextEventPatternRowDB[] queryForContextEventPatternsByCalledPeersID(long calledPeerID) {
	Log.d(TAG, "Is about to query for CalledPeers CalledPeerID [" + calledPeerID + "]");

	List<ContextEventPatternRowDB> contextEventPatternRows = new ArrayList<ContextEventPatternRowDB>();

	Cursor cursor = database.query(ContextEventPatternsTable.TABLE_NAME,
		ContextEventPatternsTable.allColumns,
		ContextEventPatternsTable.COLUMN_CALLED_PEERS_FK + " = ?",
		new String[] { String.valueOf(calledPeerID) }, null, null, null);
     ...
     * 
     */
    public FiltererContainerRowDB addFiltererContainer(String containerKey, String containerType) {
	FiltererContainerRowDB row = null;

	long insertedFiltererContainer = insertFiltererContainer(containerKey, containerType);
    ...
    public FiltererContainerRowDB queryForFiltererContainer(String containerKey,
	    String containerType) {
	Log.d(TAG, "Is about to query for FiltererContainer ContainerKey [" + containerKey
		+ "]; ContainerType [" + containerType + "]");

	FiltererContainerRowDB filtererContainerRowDB = null;

	Cursor cursor = database.query(FiltererContainerTable.TABLE_NAME,
		FiltererContainerTable.allColumns, FiltererContainerTable.COLUMN_CONTAINER_KEY
			+ " = ? AND " + FiltererContainerTable.COLUMN_CONTAINER_TYPE + " = ?",
		new String[] { containerKey, containerType }, null, null, null);

	cursor.moveToFirst();

	if (!cursor.isAfterLast()) {
	    filtererContainerRowDB = cursorToFiltererContainer(cursor);
	    Log.d(TAG, "Found FiltererContainer [" + containerKey + ";" + containerType + "]");
	} else {
	    Log.d(TAG, "No FiltererContainer was found [" + containerKey + ";" + containerType
		    + "]");
	}
    ...
    public FiltererContainerRowDB[] queryForFiltererContainersByContainerType(String containerType) {
	Log.d(TAG, "Is about to query for FiltererContainers ContainerType [" + containerType + "]");

	List<FiltererContainerRowDB> filtererContainerRows = new ArrayList<FiltererContainerRowDB>();

	Cursor cursor = database.query(FiltererContainerTable.TABLE_NAME,
		FiltererContainerTable.allColumns, FiltererContainerTable.COLUMN_CONTAINER_TYPE
    ...
    private long insertFiltererContainer(String containerKey, String containerType) {
	Log.d(TAG, "Is about to insert FiltererContainer for ContainerKey [" + containerKey
		+ "]; ContainerType [" + containerType + "]");

	// Populate the values to insert
	ContentValues valuesForFiltererContainer = new ContentValues();
	valuesForFiltererContainer.put(FiltererContainerTable.COLUMN_CONTAINER_KEY, containerKey);
	valuesForFiltererContainer.put(FiltererContainerTable.COLUMN_CONTAINER_TYPE, containerType);

	// Insert the new row to the FiltererContainer table
	long insertedID = database.insert(FiltererContainerTable.TABLE_NAME, null,
		valuesForFiltererContainer);
	Log.d(TAG, "Inserted status [" + insertedID + "] to [" + FiltererContainerTable.TABLE_NAME
		+ "] table");

	return insertedID;
    }

    private long queryForFiltererContainerID(String containerKey, String containerType) {
	// Query for the FiltererContainer
	FiltererContainerRowDB filtererContainer = queryForFiltererContainer(containerKey,
		containerType);
     ...
     * 
     */
    public void addContextFilterer(String containerKey, String containerType, String groundingID,
	    String serializedContextEvent) {
	// Query
	long filtererContainerID = queryForFiltererContainerID(containerKey, containerType);

	// Add the filterer
	insertContextFilterer(filtererContainerID, groundingID, serializedContextEvent);
    }

    public ContextFiltererRowDB[] queryForContextFilterers(String containerKey, String containerType) {
    ...
    public void removeContextFilterers(String containerKey, String containerType,
	...
	// Remove the filterer
	removeContextFilterers(filtererContainerID, contextSubscriberGroundingID);
    }

    private long insertContextFilterer(long filtererContainerID, String groundingID,
	    String serializedContextEvent) {
	Log.d(TAG, "Is about to insert ContextFilterer for FiltererContainerID ["
		+ filtererContainerID + "]; GroundingID [" + groundingID + "]");

	// Populate the values to insert
	ContentValues valuesForContextFilterer = new ContentValues();
	valuesForContextFilterer.put(ContextFiltererTable.COLUMN_FILTERER_CONTAINER_FK,
		filtererContainerID);
	valuesForContextFilterer.put(ContextFiltererTable.COLUMN_GROUNDING_ID, groundingID);
	valuesForContextFilterer.put(ContextFiltererTable.COLUMN_CONTEXT_EVENT_CONTENT,
		serializedContextEvent);

	// Insert the new row to the ContextFilterer table
	long insertedID = database.insert(ContextFiltererTable.TABLE_NAME, null,
		valuesForContextFilterer);
	Log.d(TAG, "Inserted status [" + insertedID + "] to [" + ContextFiltererTable.TABLE_NAME
		+ "] table");
	...
	Log.d(TAG, "Is about to query for ContextFilterers FiltererContainerID ["
		+ filtererContainerID + "]");
	...
	Cursor cursor = database.query(
		...
		ContextFiltererTable.allColumns, // TODO: make sure that the
						 // search by ID as string will
						 // work here...
		ContextFiltererTable.COLUMN_FILTERER_CONTAINER_FK + " = " + filtererContainerID,
		null, null, null, null);
    ...
    private void removeContextFilterers(long filtererContainerID,
	    String contextSubscriberGroundingID) {
	Log.d(TAG, "Is about to remove ContextFilrerers with FiltererContainerID ["
		+ filtererContainerID + "]; SubscriberID [" + contextSubscriberGroundingID + "]");

	int numOfDeletedRows = database.delete(ContextFiltererTable.TABLE_NAME,
		ContextFiltererTable.COLUMN_CONTEXT_FILTERER_ID_PK + "= ? AND "
			+ ContextFiltererTable.COLUMN_GROUNDING_ID + "= ?",
		new String[] { String.valueOf(filtererContainerID), contextSubscriberGroundingID });
	...
	Cursor cursor = database.query(AllProvisionsTable.TABLE_NAME,
		AllProvisionsTable.allColumns, null, null, null, null, null);
	...
	Log.d(TAG, "Is about to insert AllProvision ContextEventPatternAsString ["
		+ contextEventPatternAsString + "]");

	// Populate the values to insert
	ContentValues valuesForAllProvisions = new ContentValues();
	valuesForAllProvisions.put(AllProvisionsTable.COLUMN_CONTEXT_EVENT_PATTERN_CONTENT,
		contextEventPatternAsString);

	// Insert the new row to the AllProvisions table
	long insertedID = database.insert(AllProvisionsTable.TABLE_NAME, null,
		valuesForAllProvisions);
	Log.d(TAG, "Inserted status [" + insertedID + "] to [" + AllProvisionsTable.TABLE_NAME
		+ "] table");
