...
public class RegisterContextsHandler extends AbstractMessagePersistableHandler {
    ...
    private static final String TAG = RegisterContextsHandler.class.getCanonicalName();
	...
	if (registerContextsMessage.getAndroidContextPublisherDataWrapperList().isEmpty()
		&& registerContextsMessage.getAndroidContextSubscriberDataWrapperList().isEmpty()) {
	...
	AndroidContextBus androidContextBus = new AndroidContextBus(sodaPop, context);
	...
	for (AndroidContextPublisherDataWrapper contextPublisherRegistration : registerContextsMessage
	    ...
	    handleContextPublisher(contextPublisherRegistration, androidContextBus, context);
	}
	...
	for (AndroidContextSubscriberDataWrapper contextSubscriberRegistration : registerContextsMessage
	    ...
	    handleContextSubscriber(contextSubscriberRegistration, androidContextBus, context);
	}
    }

    private void handleContextPublisher(
	    AndroidContextPublisherDataWrapper contextPublisherRegistration,
	    AndroidContextBus androidContextBus, Context context) {
	...
	// registration stuff
	new AndroidContextPublisherProxy(androidContextBus, true, contextProviderInfo,
		contextPublisherRegistration.getPackageName(),
		contextPublisherRegistration.getGroundingID(),
		contextPublisherRegistration.getAndroidUniqueName(), contextPublisherGroundingXml,
		context);
    }
	...
	registerOntologies(contextSubscriberGroundingXml.getOntologies());
	...
	try {
	    ...
	    String errMsg = e.getMessage() + " for PackageName ["
		    + contextSubscriberRegistration.getPackageName() + "]; GroundingID ["
		    + contextSubscriberRegistration.getGroundingID() + "]; AndroidUniqueName ["
	...
	// registration stuff
	new AndroidContextSubscriberProxy(androidContextBus, true, initialSubscriptions,
	...
	for (IRestrictionXmlObj restriction : action.getRestrictions()) {
	    mergedRestriction = restriction.populateRestriction(mergedRestriction);
	}
