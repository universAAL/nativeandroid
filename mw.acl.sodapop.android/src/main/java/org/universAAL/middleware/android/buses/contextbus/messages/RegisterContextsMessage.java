...
public class RegisterContextsMessage extends AbstractRegisterAppsMessage {
    ...
    private static final String TAG = RegisterContextsMessage.class.getCanonicalName();
	...
	scanMetadataForContextsAndUpdateList(metaData, componentNameString, pm, appInfo);
    }
	...
	int contextPublisherGroundingResourceID = extractResourceFromMetadata(metadata,
		IAndroidSodaPop.META_DATA_TAG_CONTEXT_PUBLISHER_GROUNDING);
	int contextSubscriberGroundingResourceID = extractResourceFromMetadata(metadata,
		IAndroidSodaPop.META_DATA_TAG_CONTEXT_SUBSCRIBER_GROUNDING);
	...
	if (0 != contextPublisherGroundingResourceID) {
	    InputStream is = resources.openRawResource(contextPublisherGroundingResourceID);
	    ...
	    // Add to the list
	    androidContextPublisherDataWrapperList.add(new AndroidContextPublisherDataWrapper(
		    packageName, androidUniqueName, contextPublisherGroundingXml));

	    Log.d(TAG, "Found ContextPublisherGrounding for [" + androidUniqueName + "]");
	}

	if (0 != contextSubscriberGroundingResourceID) {
	    InputStream is = resources.openRawResource(contextSubscriberGroundingResourceID);
	    ...
	    // Add to the list
	    androidContextSubscriberDataWrapperList.add(new AndroidContextSubscriberDataWrapper(
		    packageName, androidUniqueName, contextSubscriberGroundingXml));

	    Log.d(TAG, "Found ContextSubscriberGrounding for [" + androidUniqueName + "]");
	}
    }

    public static class AndroidContextPublisherDataWrapper extends AbstractGroundingDataWrapper {

	private ContextPublisherGroundingXmlObj contextPublisherGroundingXmlObj;

	public AndroidContextPublisherDataWrapper(String packageName, String androidUniqueName,
    ...
    public static class AndroidContextSubscriberDataWrapper extends AbstractGroundingDataWrapper {

	private ContextSubscriberGroundingXmlObj contextSubscriberGroundingXmlObj;

	public AndroidContextSubscriberDataWrapper(String packageName, String androidUniqueName,
