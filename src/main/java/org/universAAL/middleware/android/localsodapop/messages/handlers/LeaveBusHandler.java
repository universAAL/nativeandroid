 ...
 * 
 */
public class LeaveBusHandler extends AbstractMessagePersistableHandler {

    @Override
    protected void privateHandleMessage(IMessage message, AbstractSodaPopAndroidImpl sodaPop) {
	// Cast it to leave bus message
	LeaveBusMessage leaveBusMessage = (LeaveBusMessage) message;

	// Remove the bus
	sodaPop.leaveBus(leaveBusMessage.getBusName(), leaveBusMessage.getPeerID());
    }
}
