...
public class ServiceBusSQLiteMngr extends AbstractCommonBusSQLiteMngr {
    ...
    private static final String[] tablesToClearInReset = new String[] {
	    RegistryServicesTable.TABLE_NAME, LocalServiceSearchResultsTable.TABLE_NAME,
	    LocalServicesIndexTable.TABLE_NAME, LocalWaitingCallerTable.TABLE_NAME,
	    ServiceProfileTable.TABLE_NAME, WaitingCallTable.TABLE_NAME };
    ...
    public void addLocalServiceIndex(String processURI, String serviceRealizationAsResource) {
	LocalServiceIndexRowDB localServiceIndex = queryLocalServiceIndexByProcessURI(processURI);

	if (null == localServiceIndex) {
	    long insertedLocalServiceIndex = insertLocalServiceIndex(processURI,
		    serviceRealizationAsResource);

	    Log.d(TAG, "LocalServiceIndex was inserted successfully [" + insertedLocalServiceIndex
		    + "]");
	} else {
	    Log.w(TAG, "No need to insert a new one, LocalServiceIndex already exists for ["
		    + processURI + "]");
	}
    }

    public LocalServiceIndexRowDB removeLocalServiceIndexByProcessURI(String processURI) {
	...
	int numOfDeletedRows = database.delete(LocalServicesIndexTable.TABLE_NAME,
		LocalServicesIndexTable.COLUMN_PROCESS_URI + " = ?", new String[] { processURI });
    ...
    public LocalServiceIndexRowDB queryLocalServiceIndexByProcessURI(String processURI) {
	LocalServiceIndexRowDB localServiceIndexRowDB = null;

	Cursor cursor = database.query(LocalServicesIndexTable.TABLE_NAME,
		LocalServicesIndexTable.allColumns, LocalServicesIndexTable.COLUMN_PROCESS_URI
	...
	Cursor cursor = database.query(LocalServicesIndexTable.TABLE_NAME,
		LocalServicesIndexTable.allColumns, null, null, null, null, null);
    ...
    private long insertLocalServiceIndex(String processURI, String serviceRealizationAsResource) {
	Log.d(TAG, "Is about to insert LocalServiceIndex processURI [" + processURI
		+ "]; ServiceRealizationAsResource [" + serviceRealizationAsResource + "]");

	// Populate the values to insert
	ContentValues valuesForLocalServicesIndexTable = new ContentValues();
	valuesForLocalServicesIndexTable
		.put(LocalServicesIndexTable.COLUMN_PROCESS_URI, processURI);
	...
	return database.insert(LocalServicesIndexTable.TABLE_NAME, null,
		valuesForLocalServicesIndexTable);
    }
    ...
    public void addLocalServicesSearchResultsWithProfiles(String serviceRealizationID,
	...
	for (String serializedProfile : serializedProfiles) {
	    long insertedServiceProfile = insertServiceProfile(serializedProfile,
		    insertedLocalServicesSearchResults);

	    Log.d(TAG, "ServiceProfile was inserted successfully [" + insertedServiceProfile
		    + "] with [" + insertedLocalServicesSearchResults + "] reference");
	}
    }
	...
	Log.d(TAG, "Is about to insert LocalServicesSearchResults serviceRealizationID ["
		+ serviceRealizationID + "]");

	// Populate the values to insert
	ContentValues valuesLocalServicesSearchResultsTable = new ContentValues();
	valuesLocalServicesSearchResultsTable.put(
		LocalServiceSearchResultsTable.COLUMN_SERVICE_REALIZATION_ID, serviceRealizationID);

	// Insert the new row to the LocalServicesIndex table
	return database.insert(LocalServiceSearchResultsTable.TABLE_NAME, null,
     ...
     * 
     */
    private long insertServiceProfile(String serializedProfile, long localServicesSearchResultsID) {
	Log.d(TAG, "Is about to insert ServiceProfile SerializedProfile [" + serializedProfile
		+ "]; LocalServicesSearchResultsID [" + localServicesSearchResultsID + "]");

	// Populate the values to insert
	ContentValues valuesServiceProfileTable = new ContentValues();
	valuesServiceProfileTable.put(ServiceProfileTable.COLUMN_CONTENT, serializedProfile);
	valuesServiceProfileTable.put(ServiceProfileTable.COLUMN_SERVICES_ID_FK,
		localServicesSearchResultsID);

	// Insert the new row to the ServiceProfile table
	return database.insert(ServiceProfileTable.TABLE_NAME, null, valuesServiceProfileTable);
    }
	...
	Log.d(TAG, "Is about to insert LocalWaiter MsgID [" + msgID + "]; CallerID [" + callerID
		+ "]");

	// Populate the values to insert
	ContentValues valuesLocalWaitingCallerTable = new ContentValues();
	valuesLocalWaitingCallerTable.put(LocalWaitingCallerTable.COLUMN_MESSAGE_ID, msgID);
	valuesLocalWaitingCallerTable.put(LocalWaitingCallerTable.COLUMN_CALLER_ID, callerID);

	// Insert the new row to the LocalWaitingCaller table
	long id = database.insert(LocalWaitingCallerTable.TABLE_NAME, null,
		valuesLocalWaitingCallerTable);
	...
	if (null != localWaitingCaller) {
	    int numOfDeletedRows = database.delete(LocalWaitingCallerTable.TABLE_NAME,
		    LocalWaitingCallerTable.COLUMN_MESSAGE_ID + " = ?", new String[] { msgID });
    ...
    private LocalWaitingCallerRowDB queryForLocalWaitingCallerByMessageID(String msgID) {
	Log.d(TAG, "Is about to query for LocalWaitingCaller MessageID [" + msgID + "]");

	LocalWaitingCallerRowDB localWaitingCallerRowDB = null;

	Cursor cursor = database.query(LocalWaitingCallerTable.TABLE_NAME,
		LocalWaitingCallerTable.allColumns, LocalWaitingCallerTable.COLUMN_MESSAGE_ID
    ...
    public void addWaitingCall(String callID, String actionName, String replyToAction,
	    String replyToCategory) {
	Log.d(TAG, "Is about to insert WaitingCall CallID [" + callID + "]; ActionName ["
		+ actionName + "]; ReplyToAction [" + replyToAction + "]; ReplyToCategory ["
		+ replyToCategory + "]");
	...
	valuesWaitingCallsTable.put(WaitingCallTable.COLUMN_ACTION_NAME, actionName);
	valuesWaitingCallsTable.put(WaitingCallTable.COLUMN_REPLY_TO_ACTION, replyToAction);
	valuesWaitingCallsTable.put(WaitingCallTable.COLUMN_REPLY_TO_CATEGORY, replyToCategory);

	// Insert the new row to the WaitingCalls table
	long id = database.insert(WaitingCallTable.TABLE_NAME, null, valuesWaitingCallsTable);
	...
	if (null != waitingCallRowDB) {
	    int numOfDeletedRows = database.delete(WaitingCallTable.TABLE_NAME,
		    WaitingCallTable.COLUMN_CALL_ID + " = ?", new String[] { callID });
	...
	Cursor cursor = database
		.query(WaitingCallTable.TABLE_NAME, WaitingCallTable.allColumns,
			WaitingCallTable.COLUMN_CALL_ID + " = ?", new String[] { callID }, null,
			null, null);
	...
	while (!cursor.isAfterLast()) {
		...
		localServiceSearchResultRowDB = localServiceSearchResultRowDBMap.get(id);
	    } else {
		localServiceSearchResultRowDB = cursorLocalServiceSearchResult(cursor);
		localServiceSearchResultRowDBMap.put(id, localServiceSearchResultRowDB);
	    }
	    // Add Service Profile
    ...
    private LocalServiceSearchResultRowDB cursorLocalServiceSearchResult(Cursor cursor) {
	LocalServiceSearchResultRowDB localServiceSearchResultRowDB = new LocalServiceSearchResultRowDB();
	localServiceSearchResultRowDB.setServiceRealizationID(cursor.getString(0));
