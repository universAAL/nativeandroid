...
public class ServiceCallerRequestBroadcastReceiver extends BroadcastReceiver {
    ...
    public ServiceCallerRequestBroadcastReceiver(String serviceCallerID, String androidAction) {
	...
	Log.d(TAG, "ServiceCallerRequestBroadcastReceiver received Intent [" + intent.toString()
		+ "]; ServiceCallerID [" + serviceCallerID + "]");
	...
	// Change the action
	androidServiceBusIntent.setAction(IAndroidSodaPop.ACTION_SERVICE_CALLER_REQUEST);

	// Store the service caller ID
	androidServiceBusIntent.putExtra(IAndroidSodaPop.EXTRAS_KEY_SERVICE_CALLER_ID,
		serviceCallerID);

	// Store the android action (the action that is related to the caller)
	androidServiceBusIntent.putExtra(IAndroidSodaPop.EXTRAS_KEY_ACTION_NAME, androidAction);
