...
import org.universAAL.middleware.android.buses.contextbus.contextsubscriber.xml.objects.factory.RestrictionFactory;
...
public class ActionXmlObj implements Serializable {
	...
	predicatePropName = CommonXmlParserUtils.getAttributeValueAsString(actionNode,
	...
	Node restrictionsNode = CommonXmlParserUtils.getSingleChildNode(actionNode,
		IContextSubscriberGroundingXmlConstants.RESTRICTIONS_ELEMENT);

	for (RestrictionType restrictionType : RestrictionType.values()) {
	    List<Node> restrictionNodes = CommonXmlParserUtils.getChildNodes(restrictionsNode,
		    restrictionType.toString());

	    for (Node curRestrictionNode : restrictionNodes) {
		restrictions.add(RestrictionFactory.createRestriction(restrictionType,
			curRestrictionNode));
	    }
	}
    }
	...
	Node subjectFiltersNode = CommonXmlParserUtils.getSingleChildNode(actionNode,
		IContextSubscriberGroundingXmlConstants.SUBJECT_FILTERS_ELEMENT);

	List<Node> subjectFilterNodes = CommonXmlParserUtils.getChildNodes(subjectFiltersNode,
