...
public class ServiceRequestGroundingXmlObj extends OntologiesContainedXml {
	...
	// Get the version
	version = CommonXmlParserUtils.getAttributeValueAsString(serviceRequestGroundingNode,
	...
	return (ServiceRequestGroundingXmlObj) SerializeUtils.deserializeObject(objAsBytesArr);
    }

    private void populateActions(Node serviceRequestGroundingNode) {
	// Get the actions element
	Node actionsNode = CommonXmlParserUtils.getSingleChildNode(serviceRequestGroundingNode,
		IServiceRequestGroundingXmlConstants.ACTIONS_ELEMENT);

	// Get the actions list
	List<Node> actionsChildNodes = CommonXmlParserUtils.getChildNodes(actionsNode,
