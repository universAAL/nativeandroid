...
public class ServiceCalleeResponsesBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		Log.d(TAG, "ServiceCalleResponsesBroadcastReceiver received Intent ["
				+ intent.toString() + "]; in reply to [" + replyTo + "]");
		...
		androidServiceBusIntent
				.setAction(StringConstants.ACTION_PROCESS_BUS_MESSAGE_RSP);

		// Add the serviceCalleeID + replyTo to the extras section
		androidServiceBusIntent.putExtra(
				StringConstants.EXTRAS_KEY_SERVICE_CALLEE_ID, serviceCalleeID);
		androidServiceBusIntent.putExtra(
				StringConstants.EXTRAS_KEY_OPERATION_NAME,
				operationNameToRespondTo);
		androidServiceBusIntent.putExtra(StringConstants.EXTRAS_KEY_REPLY_TO,
				replyTo);
		androidServiceBusIntent.putExtra(
