...
public class RegisterServicesMessage extends AbstractRegisterAppsMessage {
	...
	private final static String TAG = RegisterServicesMessage.class
			.getCanonicalName();
	...
	protected void scanMetadataInActivitiesAndUpdateList(Bundle metaData,
		...
		scanMetadataForServicesAndUpdateList(metaData, componentNameString, pm,
				appInfo, AndroidServiceType.ACTIVITY);
	}
	...
	protected void scanMetadataForServicesAndUpdateList(Bundle metadata,
			String componentNameString, PackageManager pm,
			ApplicationInfo appInfo, AndroidServiceType androidServiceType)
			throws NameNotFoundException {

		int serviceGroundingResourceID = extractResourceFromMetadata(metadata,
				StringConstants.META_DATA_TAG_SERVICE_GROUNDING);
		int serviceRequestGroundingResourceID = extractResourceFromMetadata(
				metadata,
				StringConstants.META_DATA_TAG_SERVICE_REQUEST_GROUNDING);
		...
		if (0 != serviceGroundingResourceID) {
			InputStream is = resources
					.openRawResource(serviceGroundingResourceID);

			ServiceGroundingXmlObj serviceGroundingXml = ServiceGroundingXmlMngr
					.populateServiceGroundingXmlObjectFromXml(is,
							androidServiceType);

			// Add to the list
			getAndroidServiceCalleesDataWrapperList().add(
					new AndroidServiceCalleeDataWrapper(packageName,
							androidServiceName, serviceGroundingXml));

			Log.d(TAG, "Found ServiceGrounding for service ["
					+ androidServiceName + "]");
		}

		if (0 != serviceRequestGroundingResourceID) {
			InputStream is = resources
					.openRawResource(serviceRequestGroundingResourceID);
			...
			getAndroidServiceCallersDataWrapperList().add(
					new AndroidServiceCallerDataWrapper(packageName,
							androidServiceName, serviceRequestGroundingXml));

			Log.d(TAG, "Found ServiceRequestGrounding for ["
					+ androidServiceName + "]");
		}
	}

	public static class AndroidServiceCalleeDataWrapper extends
			AbstractGroundingDataWrapper {

		private ServiceGroundingXmlObj serviceGroundingXmlObj;

		public AndroidServiceCalleeDataWrapper(String packageName,
	...
	public static class AndroidServiceCallerDataWrapper extends
			AbstractGroundingDataWrapper {

		private ServiceRequestGroundingXmlObj serviceRequestGroundingXmlObj;

		public AndroidServiceCallerDataWrapper(String packageName,
