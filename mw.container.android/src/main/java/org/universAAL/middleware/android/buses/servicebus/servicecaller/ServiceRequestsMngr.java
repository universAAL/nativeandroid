...
public class ServiceRequestsMngr {
	...
	private final static String TAG = ServiceRequestsMngr.class
			.getCanonicalName();

	public static void reigsterToServiceRequestGroundingActions(
			String serviceCallerID,
	...
	public static void unreigsterToServiceRequestGroundingActions(
		...
		for (ActionXmlObj action : serviceRequestGroundingXmlObj.getActions()) {
			// unregister
			UnregisterBroadcastReceivers.unregisterReceiver(
					action.getAndroidAction(), action.getAndroidCategory(),
					context);
		}
	}

	private static BroadcastReceiver registerReceiver(String serviceCallerID,
			ActionXmlObj action, Context context) {
		// Initiate the receiver
		BroadcastReceiver receiver = new ServiceCallerRequestBroadcastReceiver(
				serviceCallerID, action.getAndroidAction());
		...
		Log.d(TAG,
				"A registration to receiver for service request has been performed: Action ["
						+ action.getAndroidAction() + "]; Category ["
						+ action.getAndroidCategory() + "]");
