...
public abstract class AbstractBusMembersTable {
	...
	private String TABLE_CREATE = "create table if not exists "
			+ getTableName() + "( " + COLUMN_ID_PK + " "
			+ ISqlConstants.AUTO_INCREMENT_PK + ", " + COLUMN_MEMBER_ID
			+ " VARCHAR(50) UNIQUE " + ISqlConstants.NOT_NULL_COLUMN + ", "
			+ COLUMN_PACKAGE_NAME + " VARCHAR(50) "
			+ ISqlConstants.NOT_NULL_COLUMN + ", " + COLUMN_MEMBER_TYPE
			+ " VARCHAR(20) " + ISqlConstants.NOT_NULL_COLUMN + ", "
			+ COLUMN_GROUNDING_ID + " VARCHAR(50) UNIQUE "
			+ ISqlConstants.NOT_NULL_COLUMN + ", " + COLUMN_ANDROID_UNIQUE_NAME
			+ " VARCHAR(50) " + ISqlConstants.NOT_NULL_COLUMN + ", "
			+ COLUMN_GROUNDING_XML + " BLOB " + ISqlConstants.NOT_NULL_COLUMN
			+ ");";

	public static final String[] allColumns = new String[] { COLUMN_MEMBER_ID,
			COLUMN_PACKAGE_NAME, COLUMN_MEMBER_TYPE, COLUMN_GROUNDING_ID,
			COLUMN_ANDROID_UNIQUE_NAME, COLUMN_GROUNDING_XML };
