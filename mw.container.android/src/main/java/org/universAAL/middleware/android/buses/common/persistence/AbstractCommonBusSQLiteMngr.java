...
public abstract class AbstractCommonBusSQLiteMngr {
	...
	private static final String TAG = AbstractCommonBusSQLiteMngr.class
			.getName();
	...
	public AbstractCommonBusSQLiteMngr(Context context,
			AbstractCommonBusSQLiteHelper helper) {
		dbHelper = helper;
	}
	...
	public String queryBusMemberIDByGroundingID(String groundingID) {
		...
		synchronized (sync) {
			try {
				open();
				Cursor cursor = database.query(getRegistryTableName(),
						AbstractBusMembersTable.allColumns,
						AbstractBusMembersTable.COLUMN_GROUNDING_ID + " = ?",
						new String[] { groundingID }, null, null, null);
	...
	public BusMemberRowDB[] queryForAllBusMembers() {
		...
		synchronized (sync) {
			try {
				open();
				Cursor cursor = database.query(getRegistryTableName(),
						AbstractBusMembersTable.allColumns, null, null, null,
						null, null);
	...
	public BusMemberRowDB[] queryForBusMembersByPackageName(String packageName) {
		...
		synchronized (sync) {
			try {
				...
				Cursor cursor = database.query(getRegistryTableName(),
						AbstractBusMembersTable.allColumns,
						AbstractBusMembersTable.COLUMN_PACKAGE_NAME + " = ?",
		...
		synchronized (sync) {
			try {
				...
				int numOfDeletedRows = database.delete(getRegistryTableName(),
						AbstractBusMembersTable.COLUMN_MEMBER_ID + " = ?",
						new String[] { memberID });
	...
	public void addBusMember(String memberID, String packageName,
			String groundingID, MemberType memberType,
			String androidUniqueName, byte[] groundingXmlAsByteArr) {
		long insertedRegistryService = insertRegistryService(memberID,
				packageName, groundingID, memberType, androidUniqueName,
				groundingXmlAsByteArr);
		Log.d(TAG, "RegistryService insert status [" + insertedRegistryService
				+ "]");
	}

	private long insertRegistryService(String memberID, String packageName,
			String groundingID, MemberType memberType,
			String androidUniqueName, byte[] groundingXmlAsByteArr) {
		Log.d(TAG, "Is about to insert RegistryService MemberID [" + memberID
				+ "]; PackageName [" + packageName + "]; GroundingID ["
				+ groundingID + "]; AndroidUniqueName [" + androidUniqueName
				+ "]");

		// Populate the values to insert
		ContentValues valuesForRegistryServiceTable = new ContentValues();
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_MEMBER_ID, memberID);
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_PACKAGE_NAME, packageName);
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_GROUNDING_ID, groundingID);
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_MEMBER_TYPE,
				String.valueOf(memberType));
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_ANDROID_UNIQUE_NAME,
				androidUniqueName);
		valuesForRegistryServiceTable.put(
				AbstractBusMembersTable.COLUMN_GROUNDING_XML,
				groundingXmlAsByteArr);
		synchronized (sync) {
			try {
				open();
				// Insert the new row to the RegistryServices table
				return database.insert(getRegistryTableName(), null,
						valuesForRegistryServiceTable);
			} finally {
				close();
			}
		}
