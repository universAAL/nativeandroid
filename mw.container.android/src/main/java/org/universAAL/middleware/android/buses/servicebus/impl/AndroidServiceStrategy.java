...
import org.universAAL.middleware.android.buses.servicebus.data.AndroidServiceStrategyDataFactory;
 ...
 * 
 */
public class AndroidServiceStrategy extends BusStrategy {
	private static final String TAG ="AndroidServiceStrategy";
	...
	public AndroidServiceStrategy(CommunicationModule commModule/* , ModuleContext mc */) {
		...
		AndroidIServiceStrategyDataFactory factory = createServiceStrategyDataFactory();
		...
		Log.d(TAG, "This instance is "+ (isCoordinator ? "" : "NOT ")+
						"the coordinator.");
		if (isCoordinator) {
		...
		//TODO Check if this was really necessary
//		theCoordinator = queryForCoordinator();
	...
	void addAvailabilitySubscription(String callerID,
		...
		if (isCoordinator)
			...
			((AndroidServiceBusImpl) bus).assessContentSerialization(res);
	...
	void addRegParams(String calleeID, ServiceProfile[] realizedServices) {
		...
		if (!isCoordinator && isCoordinatorKnown()) {
			...
			((AndroidServiceBusImpl) bus).assessContentSerialization(r);
	...
	private void callServices(BusMessage m, Vector matches) {
		...
		for (int i = 0; i < size; i++) {
			...
			((AndroidServiceBusImpl) bus).assessContentSerialization(sc);
			...
			try {
				...
				if (call == null) {
					Log.e(TAG,  "Call is null - ignoring." );
				} else if (call.getSender() == null) {
					Log.e(TAG, "Call.getSender() is null - ignoring." );
				} else if (call.getSender().getPeerID() == null) {
					Log.e(TAG, "Call.getSender().getPeerID() is null - ignoring." );
				}

				if (receiver == null) {
					Log.e(TAG,  "Receiver is null - ignoring." );
				} else if (receiver.getPeerID() == null) {
					Log.e(TAG,  "Receiver.getPeerID() is null - ignoring." );
				}
	...
	private void sendServiceResponse(BusMessage m) {
		...
		((AndroidServiceBusImpl) bus).assessContentSerialization((Resource) m
				.getContent());
		if (m.receiverResidesOnDifferentPeer()) {
			send(m);
		} else {
				...
				synchronized (allServicesIndex) {
					...
					if (v == null)
						...
						Long logID = Long.valueOf(Thread.currentThread()
								.getId());
						Log.v(TAG, ServiceBus.LOG_MATCHING_START+
						...
						for (Iterator i = v.iterator(); i.hasNext();) {
							...
							Log.v(TAG, ServiceBus.LOG_MATCHING_PROFILE+
											profileService.getType()+
											profileServiceURI+ logID );
							Hashtable context = matches(caller, request, sr,
									logID);
							if (context != null) {
								matches.add(context);
								Log.v(TAG, ServiceBus.LOG_MATCHING_SUCCESS+
												logID);
							} else
								Log.v(TAG, ServiceBus.LOG_MATCHING_NOSUCCESS+
												logID );
						}
						Log.v(TAG, ServiceBus.LOG_MATCHING_END+"found "+
	...
	private void replyToInitialDialogInfoRequest(BusMessage m,
		...
		((AndroidServiceBusImpl) bus).assessContentSerialization(resp);
		m = m.createReply(resp);
		if (m.receiverResidesOnDifferentPeer())
			send(m);
		else
	...
	private void replyToLocalCaller(BusMessage msg) {
		...
		if (replyOf == null) {
			...
			// => ignore!
			Log.d(TAG,  "Message of type REPLY, but not containing inReplyTo. Ignoring it." );
		} else {
			...
			if (callerID == null) {
				...
				// => ignore!
				Log.d(TAG,  "There is no caller. To whom should I then reply? Ignoring it." );
			} else {
	...
	protected void waitForCoordinatorToBeKnown() throws InterruptedException {
		...
		for (int i = 0; i < AMOUNT_OF_COORDINATOR_CHECKS_TRIALS; i++) {
			...
			PeerCard coordinator = queryForCoordinator();
			if (null != coordinator) {
...
//		AbstractSodaPopAndroidImpl androidSodapop = (AbstractSodaPopAndroidImpl) sodapop;
	...
	public void sendResponse(String messageIDInReplyTo, String serviceCalleeID,
		...
		String memberID = serviceCalleeID;/*
				.substring(Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX.length());//TODO: Check with/out prefix*/
		AndroidServiceCalleeProxy androidServiceCallee = (AndroidServiceCalleeProxy) getBusMember(memberID);

		// Handle the response
		androidServiceCallee.handleResponse(messageIDInReplyTo,
				operationNameToRespondTo, replyTo, extras);
	}

	public void saveCall(String callID, String actionName,
			String replyToAction, String replyToCategory) {
		// Get the interface
		IWaitingCallsData waitingCallsData = getWaitingCallsData();

		// Add the waiting call
		waitingCallsData.addWaitingCall(callID, actionName, replyToAction,
				replyToCategory);
	}
