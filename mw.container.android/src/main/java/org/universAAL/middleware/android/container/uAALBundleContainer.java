...
import org.universAAL.middleware.android.container.ContextEmulator.ServiceEvent;
 ...
 * 
 */
public class uAALBundleContainer implements Container {
	...
	public ModuleContext registerModule(Object[] regParams) {
		ContextEmulator bContext = null;
		if (regParams != null && regParams.length > 0
				&& regParams[0] instanceof ContextEmulator) {
			bContext = (ContextEmulator) regParams[0];
			// register me as ServiceListener
			bContext.addServiceListener(THE_CONTAINER);
		}
		if(regParams.length>1){
			return (regParams != null && regParams.length > 0 && regParams[0] instanceof ContextEmulator) ? new uAALBundleContext(
					(ContextEmulator) regParams[0], (String)regParams[1]) : null;//Need a second param for the id
		}else{
			return (regParams != null && regParams.length > 0 && regParams[0] instanceof ContextEmulator) ? new uAALBundleContext(
					(ContextEmulator) regParams[0]) : null;
		}	
	}
	 ...
	 */
	public void serviceChanged(ContextEmulator.ServiceEvent se) {
		// NOTE: Not handling ServiceEvent.MODIFIED event
		if (se == null || se.getType() == ServiceEvent.MODIFIED)
			return;

		final ContextEmulator bc = se.getContext();
		...
		case ServiceEvent.REGISTERED:
			for (Iterator i = listenersLocalCopy.iterator(); i.hasNext();)
				((SharedObjectListener) i.next()).sharedObjectAdded(
						bc.getService(se.getService()), se.getService());
			break;
		case ServiceEvent.UNREGISTERING:
			for (Iterator i = listenersLocalCopy.iterator(); i.hasNext();) {
				SharedObjectListener sol = (SharedObjectListener) i.next();
				if (sol != null && bc != null) {
					sol.sharedObjectRemoved(bc.getService(se.getService()));
				}
			}
			break;
		}
