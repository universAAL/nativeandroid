...
public abstract class AbstractRegisterAppsMessage extends AbstractMessage {
	...
	protected void populateAppsList() {
		...
		Log.d(TAG, "Is about to search for services registrtion in package ["
				+ packageName + "]");
		...
		try {
			...
			scanMetadataInApplicationInfoAndUpdateList(appInfo.metaData,
					componentNameString, pm, appInfo);
			...
			if (packInfo.activities != null) {
				for (int i = 0; i < packInfo.activities.length; i++) {
					...
					ActivityInfo ai = pm.getActivityInfo(componentName,
							PackageManager.GET_META_DATA);
					scanMetadataInActivitiesAndUpdateList(ai.metaData,
							componentNameString, pm, appInfo);
				}
			}
			...
			if (packInfo.services != null) {
				for (int i = 0; i < packInfo.services.length; i++) {
					...
					scanMetadataInServicesAndUpdateList(si.metaData,
							componentNameString, pm, appInfo,
			...
			Log.e(TAG, "Error when scanning package [" + packageName
					+ "]; Error [" + e.getMessage() + "]");
		}
	}

	protected abstract void scanMetadataInApplicationInfoAndUpdateList(
			Bundle metaData, String componentNameString, PackageManager pm,
			ApplicationInfo appInfo) throws NameNotFoundException;

	protected abstract void scanMetadataInServicesAndUpdateList(
			Bundle metaData, String componentNameString, PackageManager pm,
			ApplicationInfo appInfo, AndroidServiceType service)
			throws NameNotFoundException;

	protected abstract void scanMetadataInActivitiesAndUpdateList(
			Bundle metaData, String componentNameString, PackageManager pm,
			ApplicationInfo appInfo) throws NameNotFoundException;
