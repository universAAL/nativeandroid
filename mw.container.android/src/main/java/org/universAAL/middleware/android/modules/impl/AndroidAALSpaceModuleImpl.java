 ...
 */
public class AndroidAALSpaceModuleImpl implements AALSpaceModule,
		MessageListener, SharedObjectListener, ServiceListener, Broker {
	...
	public AndroidAALSpaceModuleImpl(ConnectorDiscWrapper wrapDiscovery,
	...
	public List<AALSpaceCard> getAALSpaces(Dictionary<String, String> filters)
		...
		if (init()) {
			Log.d(TAG,
					"Searching for the AALSpace with filters: "
							+ filters.toString() + "...");
			try {
	...
	public synchronized void newAALSpace(AALSpaceCard aalSpaceCard)
		...
		if (init()) {
			...
			Log.w(TAG,
					"AALSpaceModule cannot be initialized. Returning no AALSpace");
			throw new AALSpaceModuleException(
					AALSpaceModuleErrorCode.NO_DISCOVERY_CONNECTORS,
					"AALSpaceModule cannot be initialized. Returning no AALSpaces");
		}
	...
	public synchronized void joinAALSpace(PeerCard spaceCoordinator,
		...
		if (spaceCoordinator != null && spaceCard != null) {
			Log.d(TAG, "Peer: " + spaceCoordinator.toString()
					+ " is joining the spaceCard: " + spaceCard.toString());
			try {
				if (init()) {
					Log.d(TAG, "Creating a new AALSpaceMessage");
					// prepare the AALSpace Message
	...
	public void announceNewPeer(AALSpaceCard spaceCard, PeerCard peerCard) {
		if (spaceCard != null && peerCard != null) {
			Log.d(TAG, "Announcing new Peer: " + peerCard.toString()
					+ " into the AASpace: " + spaceCard.toString());
	...
	public void messageFromSpace(AALSpaceMessage message, PeerCard sender)
		...
		try {
			...
			case JOIN_REQUEST: {
				...
				Log.d(TAG, "Join request: " + message.toString());
			...
			case JOIN_RESPONSE: {
				...
				Log.d(TAG, "Join response: " + message.toString());
			...
			case NEW_PEER: {
				...
				Log.d(TAG, "New Peer added: " + message.toString());
				if (sender != null) {
			...
			case LEAVE: {
				...
				Log.d(TAG, "Leave request: " + message.toString());
				if (sender != null) {
			...
			case REQUEST_TO_LEAVE: {
				...
				Log.d(TAG, "Request to leave: " + message.toString());
				if (message.getSpaceDescriptor() != null) {
			...
			case REQUEST_PEERCARD: {
				Log.d(TAG, "Request PeerCard: " + message.toString());
				// check if everything is correct
				if (controlBoker.getmyAALSpaceDescriptor() == null) {
					Log.d(TAG,
							"AALSpaceDescritor not yet ready.. "
									+ message.toString());
					return;
			...
			case PEERCARD: {
				...
				Log.d(TAG, "PeerCard received: " + message.toString());
	...
	public void configureAALSpaceChannel(String group) {
		Log.d(TAG, "Setting the broker group for the AALSpaceModule..." + group);
		...
		if (message == null /* && */|| message.getContent() == null) {
			...
			try {
				...
				try {
					...
					Log.e(TAG,
							"The message received cant unmarshal...dropping it.");
				}
				...
				default:
					Log.e(TAG,
							"The message received is not valid...dropping it.");
					break;
				}
			} catch (Exception e) {
				Log.e(TAG, "Error during message receive: " + e.toString());
			}
	...
	public void handleSendError(ChannelMessage message,
		...
		try {
			Log.w(TAG,
					"Error during message queuing for message: "
							+ message.toString());
	...
	public void destroyAALSpace(AALSpaceCard spaceCard) {
		Log.d(TAG, "Destroy the AALSpace: " + spaceCard.toString());
		// to de-register the AALSpace
		try {
			wrapDiscovery.deregisterAALSpace(spaceCard);
		} catch (Exception e) {
			Log.e(TAG, "Error during destroy AALSpace: " + e.toString());
		}
	}
	...
	public void sharedObjectAdded(Object arg0, Object arg1) {
		if (arg0 != null) {
			if (arg0 instanceof DiscoveryConnector) {// TODO Not really going to
														// happen
				wrapDiscovery.addAALSpaceListener(this);
	...
	public void renewAALSpace(AALSpaceCard spaceCard) {
		if (spaceCard != null) {
			try {
				...
				Log.e(TAG,
						"error during AALSpace renew: " + spaceCard.toString(),
						e);
			}
		}
	}
