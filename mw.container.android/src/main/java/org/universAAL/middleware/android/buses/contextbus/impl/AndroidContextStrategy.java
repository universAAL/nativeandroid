...
import org.universAAL.middleware.android.buses.contextbus.data.AndroidContextStrategyDataFactory;
 ...
 * 
 */
public class AndroidContextStrategy extends BusStrategy {
	private static final String TAG = "AndroidContextStrategy";
	...
	ContextEventPattern[] getAllProvisions(ContextPublisher cs) {
		...
		synchronized (cs) {
			...
			((AndroidContextBusImpl) bus).assessContentSerialization(r);
	...
	public void handle(BusMessage message, String senderID) {
		...
		case MessageType.P2P_EVENT:
			Log.w(TAG, "Unexpected P2P_EVENT message ignored!");
			break;
		...
		case MessageType.REPLY:
			Log.w(TAG, "Unexpected Reply message ignored!");
			break;
		case MessageType.REQUEST:
			Log.w(TAG, "Unexpected Request message ignored!");
			break;
		default:
			Log.w(TAG, "Message of unknown type ignored!");
			break;
		}
	}
			...
			if (isContextBusProvisionList(resource)) {
				...
				if (provisionList instanceof List
					...
					if (calledPeers != null) {
						...
						Log.d(TAG,
								"Ignoring peer provisions received after timeout!");
					}
				} else {
					Log.d(TAG,
							"Ignoring a P2P-Reply not containing any peer provisions!");
				}
			}
		} else {
			Log.w(TAG, "P2P-Reply to handle does not contain peer provisions!");
		}
	}
	...
	private void handleEvent(BusMessage message) {
		if (!(message.getContent() instanceof ContextEvent)) {
			Log.w(TAG, "Event to handle is no instance of ContextEvent!");
			return;
		}
	...
	// THis method added by port
	protected void handleEvent(ContextSubscriber contextSubscriber,
			BusMessage msg) {
		// Cast to bus member
