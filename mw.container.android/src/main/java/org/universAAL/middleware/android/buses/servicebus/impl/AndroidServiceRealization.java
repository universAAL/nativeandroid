 ...
 * 
 */
public class AndroidServiceRealization extends FinalizedResource {
	private static final String TAG = "AndroidServiceRealization";
	...
	public AndroidServiceRealization() {
		super();
		addType(MY_URI, true);
	}

	public AndroidServiceRealization(String theCallee, ServiceProfile theProfile) {
		super();
		addType(MY_URI, true);
	...
	public AndroidServiceRealization(String instanceURI) {
		super(instanceURI);
		addType(MY_URI, true);
	}
	...
	public boolean matches(ServiceRequest request, Hashtable context, Long logID) {
				...
				if (offInsRestr == null)
					if (offClsRestr == null) {
						...
						if (reqRestr.matches(offClsRestr, context))
							...
							// successful
							context.put(
									AndroidServiceStrategy.CONTEXT_SPECIALIZED_CLASS_MATCH,
									Boolean.TRUE);
					...
					if (reqRestr.matches(offInsRestr, context))
						...
						context.put(
								AndroidServiceStrategy.CONTEXT_SPECIALIZED_INSTANCE_MATCH,
								Boolean.TRUE);
		...
		// check effects
		if (!ProcessResult.checkEffects(request.getRequiredEffects(),
				prof.getEffects(), cloned, logID))
			return false;
		...
		try {
			...
			Log.e(TAG,
					"Exception occured when trying to get non-semantic parameters from AapiServiceRequest",
					ex);
		}
