...
public class ContextBusHandlerFactory {
	...
	private static final String TAG = ContextBusHandlerFactory.class
			.getCanonicalName();
	...
	public static IMessageHandler createHandler(String action,
		...
		case INITIALIZE:
			...
			break;
		case REGISTER:
			handler = new RegisterContextsHandler(wrapper);
			break;
		case UNREGISTER:
			handler = new UnregisterContextsHandler(wrapper);
			break;
		case CONTEXT_PUBLISHER_REQUEST:
			handler = new ContextPublisherRequestHandler(wrapper);
			break;
		case PROCESS_BUS_MESSAGE:
			handler = new ProcessContextBusMessageRequestHandler(wrapper);
			break;
		}
