...
public class ContextFiltererTable {
	...
	public static String[] allColumns = { COLUMN_GROUNDING_ID,
			COLUMN_CONTEXT_EVENT_CONTENT };

	// ContextFilterer table creation
	public static final String TABLE_CREATE = "create table if not exists "
			+ TABLE_NAME + "( " + COLUMN_CONTEXT_FILTERER_ID_PK + " "
			+ ISqlConstants.AUTO_INCREMENT_PK + ", " + COLUMN_GROUNDING_ID
			+ " VARCHAR(50) " + ISqlConstants.NOT_NULL_COLUMN + ", "
			+ COLUMN_CONTEXT_EVENT_CONTENT + " VARCHAR(200) "
			+ ISqlConstants.NOT_NULL_COLUMN + ", "
			+ COLUMN_FILTERER_CONTAINER_FK + " integer, " + "FOREIGN KEY ("
			+ COLUMN_FILTERER_CONTAINER_FK + ") REFERENCES "
